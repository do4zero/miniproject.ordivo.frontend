...
const Controllers = {
  async loadMitra() {
    const response = await api.get(
      `/other/getprogram2?type=LEMBAGA&kategori=AQIQAH`
    );
  ...
  async loadProductOptions(mitra) {
    const response = await api.get(
      `/other/getprogram2?type=PROGRAM&kategori=AQIQAH&mitra=${mitra}`
    );
    const { data } = response.data;
  ...
  async submit() {
    ...
    if (this.$v.$invalid) {
      ...
      this.$toasted
        .show(
          `Terjadi kesalahan pengisian form, silakan cek kembali form anda.`,
          {
      ...
      try {
        ...
        if (this.isForSelf) {
          ...
          formData.append(
            'BioadataAqiqah[nama]',
            this.biodataAqiqah_nama
          );
          formData.append(
            'BioadataAqiqah[jk]',
            this.biodataAqiqah_gender
          );
          formData.append(
            'BioadataAqiqah[tempat_lahir]',
            this.biodataAqiqah_tempat_lahir
          );
          formData.append(
            'BioadataAqiqah[tgl_lahir]',
            this.biodataAqiqah_tanggal_lahir
          );
          formData.append(
            'BioadataAqiqah[tgl_pelaksanaan]',
            this.biodataAqiqah_tanggal_pelaksanaan
          );
          formData.append(
            'BioadataAqiqah[nama_ayah]',
            this.biodataAqiqah_nama_ayah
          );
          formData.append(
            'BioadataAqiqah[nama_ibu]',
            this.biodataAqiqah_nama_ibu
          );
          formData.append(
            'BioadataAqiqah[alamat]',
            this.biodataAqiqah_alamat
          );
        }
