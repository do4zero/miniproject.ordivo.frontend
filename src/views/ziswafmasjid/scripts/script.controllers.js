...
const Controllers = {
  async submit() {
    ...
    if (this.$v.$invalid) {
      ...
      try {
        ...
        formData.append('masjid_map_id', this.id);
        ...
        const result = await axios.post('/trxmsjd/sekitar', formData);
        this.submitStatus = false;
  ...
  async fetchMasjidInformation() {
    ...
    this.masjid = {
      ...
      status: data.is_claim ? 1 : 0,
      pengurus: data.pengurus ? data.pengurus.fullname : '',
      email: data.pengurus ? data.pengurus.email : '',
      kontak: data.pengurus ? data.pengurus.no_hp : '',
    };
  },
};
