...
const Controllers = {
  async loadProductOptions() {
    const response = await api.get(
      `/other/getprogram2?type=PROGRAM&kategori=ASURANSI`
    );
    ...
    if (this.$v.$invalid) {
      ...
      try {
        ...
        if (checkEmail(mailOrPhone)) {
          phone = defaultPhone;
          primaryEmail = this.emailOrNope;
        }

        const productvalue = this.productValue.split('-');

        var formData = new FormData();
        formData.append('Transaksi[typecampprog]', 'PROGRAM');
        formData.append('Transaksi[idcampprog]', productvalue[0]);
  ...
  productChangeEvent(val) {
    const currentNominal = val.split('-');
    this.nominal = parseInt(currentNominal[1]);
    this.forOptions = [
      {
        id: 'self',
        text: 'Untuk Pribadi',
      ...
      {
        id: 'self',
        text: 'Untuk Keluarga/Kerabat/Teman',
      },
      {
        id: 'other',
        text: 'Sedekah / Untuk Penerima Manfaat Masjed',
      },
    ];
  },
  ...
  forSelectEvent({ id, text }) {},
  genderChangeEvent(val) {},
  genderSelectEvent({ id, text }) {},
  resetBiodata() {
    const biodata = {
      fullname: null,
      ktp: null,
